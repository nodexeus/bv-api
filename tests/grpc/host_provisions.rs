...
type Service = api::host_provisions_client::HostProvisionsClient<super::Channel>;

#[tokio::test]
async fn responds_not_found_without_valid_id_for_get() {
    let tester = super::Tester::new().await;
    let req = api::GetHostProvisionRequest {
...
async fn responds_ok_with_valid_id_for_get() {
    ...
    let req = api::GetHostProvisionRequest { id: provision.id };
...
async fn responds_error_with_invalid_provision_for_create() {
    let tester = super::Tester::new().await;
    let req = api::CreateHostProvisionRequest {
