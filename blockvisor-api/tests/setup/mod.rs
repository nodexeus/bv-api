...
impl TestServer {
    ...
    pub fn org_claims_for<A: Into<Access>>(&self, org_id: OrgId, access: A) -> Claims {
        let resource = Resource::Org(org_id);
        let expirable = Expirable::from_now(chrono::Duration::minutes(15));
        Claims::new(resource, expirable, access.into())
    }

    pub fn org_jwt<A: Into<Access>>(&self, access: A) -> Jwt {
        let claims = self.org_claims_for(self.seed().org.id, access);
        self.cipher().jwt.encode(&claims).unwrap()
    }
    ...
    pub fn node_claims_for<A: Into<Access>>(&self, node_id: NodeId, access: A) -> Claims {
        let resource = Resource::Node(node_id);
        let expirable = Expirable::from_now(chrono::Duration::minutes(15));
        Claims::new(resource, expirable, access.into())
    }

    pub fn node_claims<A: Into<Access>>(&self, access: A) -> Claims {
        self.node_claims_for(self.seed().node.id, access)
    }

    pub fn node_jwt<A: Into<Access>>(&self, access: A) -> Jwt {
        let claims = self.node_claims(access);
        self.cipher().jwt.encode(&claims).unwrap()
    }
